...
def submit_enquiry(
    ...
    """Submit an enquiry (anyone can submit; optionally binds user_id if authenticated):
    1. Honeypot check (silent 200 if tripped, no save, no send).
    2. Save to 'enquiries' table immediately (user_id if authenticated, else None).
    ...
    client_name = message.name.strip() if message.name and message.name.strip() else (current_user.name if current_user else "Client")
    client_email = current_user.email.lower().strip() if current_user and current_user.email else message.email.lower().strip()
